...
def plot(cfg, inferencer, column_tag, selection) -> None:
    ...
    # for df in inferencer.full_df_iter():
    #     print(df[[f"chtypeadc_ch{idx:03d}" for idx in range(cfg.nch)]].iloc[0].tolist())
    ...
    # functions_plot.plot_2d_multicol_vs_var(varname_x="event_id", varname_y_template=f"adc_ch000_pedsub{column_tag}", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_event_id", method_subfolder), nbins_x=500, nbins_y=80, y_range=yrange, make_profile_plot=True)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="event_id", varname_y_template=f"adc_ch001_pedsub{column_tag}", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_event_id", method_subfolder), nbins_x=500, nbins_y=80, y_range=yrange, make_profile_plot=True)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="event_id", varname_y_template=f"adc_ch002_pedsub{column_tag}", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_event_id", method_subfolder), nbins_x=500, nbins_y=80, y_range=yrange, make_profile_plot=True)
    ...
    for erx in range(cfg.nerx):
        ...
        # functions_plot.plot_2d_multicol_vs_var(varname_x=f"cm_erx{erx:02d}_pedsub", varname_y_template=f"adc_ch196_pedsub{column_tag}", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "channel196_2d_vs_cm", method_subfolder), nbins_y=80, y_range=yrange)
    ...
    # 1d plots
    # functions_plot.plot_1d_multicol(varname_template=f"adc_ch196_pedsub{column_tag}", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "distributions_1d", method_subfolder), x_range=(-30, 300), nbins_x=150)
    # functions_plot.plot_1d_multicol(varname_template="trig_time", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "distributions_1d", method_subfolder), x_range=(0, 130), nbins_x=130)
    ...
    # functions_plot.plot_1d_multicol(varname_template="toa_ch196", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "distributions_1d", method_subfolder), x_range=(0, 1000), nbins_x=100)
    ...
    # for chidx in range(cfg.nch):
    #     functions_plot.plot_1d_multicol(varname_template=f"adc_ch{chidx:03d}_pedsub{column_tag}", value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_1d", method_subfolder), x_range=yrange, nbins_x=150, do_gauss_fit=True, gauss_p0=(1E6, 0, 3))
    ...
    # # all channels vs. unconnected
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch008_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch017_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch019_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch028_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch193_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch202_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch204_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
    # functions_plot.plot_2d_multicol_vs_var(varname_x="adc_ch213_pedsub", varname_y_template=adc_ch_pattern, value_iterator=inferencer.full_df_iter, out_root=os.path.join(plot_dir, "per_channel_2d_vs_disconnected", method_subfolder), nbins_y=80, y_range=yrange)
